...
```
Get-LocalUser | Select Name,Enabled,LastLogon,Description,SID
```

看是否存在陌生的用户名或有没有近期启用/近期登录的异常账号
...
看是否有不该在 Administrators 组里的账号或是否有伪装名字例如`Administrat0r`
...
最后也可以查安全日志：根据事件ID看有没有用户偷偷创建账户、加权、改名等操作
...
主要查看系统日志和安全日志，通过日志可以帮助我们验证对入侵过程的判断和发现其他入侵行为。但它的前提则是日志记录已开启的情况下才能获取
...
![](image/Pasted%20image%2020260701235710.png)

左侧Windows日志栏中分别有应用程序，安全，Setup，系统，Forwarded Events等日志

中间板块是所有事件的记录，点击指定事件可以在下方栏目查看事件的具体信息，

右侧是对事件的操作，包括筛选日志，查看事件属性等
##### 常见的Windows事件ID
